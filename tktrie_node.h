...
template <typename T, bool THREADED, typename Allocator, size_t FIXED_LEN, bool IS_LEAF> struct binary_node;
template <typename T, bool THREADED, typename Allocator, size_t FIXED_LEN, bool IS_LEAF> struct list_node;
template <typename T, bool THREADED, typename Allocator, size_t FIXED_LEN, bool IS_LEAF> struct pop_node;
...
struct node_base {
    ...
    bool has_eos() const noexcept {
        if constexpr (FIXED_LEN > 0) return false;
        else return eos_flag();
    }
