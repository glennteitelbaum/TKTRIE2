...
struct remove_helpers {
    ...
    using result_t = remove_result<T, THREADED, Allocator>;
    using atomic_ptr_t = atomic_node_ptr<T, THREADED, Allocator>;

    static result_t build_remove_path(builder_t& builder, atomic_ptr_t* root_slot, ptr_t root,
                                       std::string_view key) {
        result_t result;
...
private:
    static result_t& remove_from_node(builder_t& builder, atomic_ptr_t* parent_slot, ptr_t node,
        ...
        // 2. If EOS-only node, key not found
        if (node.is_eos()) {
            return result;
        }
        ...
        // 4. If no children (SKIP-only), key not found
        ...
        if (node.is_list()) {
            ...
            remove_from_node(builder, &node.list->children[idx], child, key, child_result);
        } else {
            // FULL
            if (!node.full->valid.test(c)) return result;
            ptr_t child = node.full->children[c].load();
            if (!child) return result;
            remove_from_node(builder, &node.full->children[c], child, key, child_result);
        }
        ...
        if (child_result.subtree_deleted) {
            return remove_child_from_node(builder, node, c, result);
        }
    ...
    static result_t& remove_child_from_node(builder_t& builder, ptr_t node, unsigned char c, result_t& result) {
        if (node.is_list()) {
            return remove_child_from_list(builder, node, c, result);
        } else {
            return remove_child_from_full(builder, node, c, result);
        }
    }

    static result_t& remove_child_from_list(builder_t& builder, ptr_t node, unsigned char c, result_t& result) {
        int idx = node.list->chars.find(c);
        if (idx >= 0) {
    ...
    static result_t& remove_child_from_full(builder_t& builder, ptr_t node, unsigned char c, result_t& result) {
    ...
    static result_t& rebuild_with_new_child(builder_t& builder, ptr_t node, unsigned char c,
                                             ptr_t new_child, result_t& result) {
    ...
    static result_t& convert_full_to_list(builder_t& builder, ptr_t node, result_t& result) {
        T* eos_val = node.get_eos();
        T* skip_eos_val = node.get_skip_eos();
        const std::string& skip = node.get_skip();

        ptr_t list = builder.build_list(skip, eos_val, skip_eos_val);
    ...
    static bool can_delete_node(ptr_t node) {
    ...
    static void try_collapse(builder_t& builder, ptr_t node, result_t& result) {
        if (node.is_eos()) return;
        if (node.get_eos()) return;
        if (node.get_skip_eos()) return;

        if (!node.is_list() && !node.is_full()) return;
